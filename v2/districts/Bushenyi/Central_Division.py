...
def populate_database():
    ...
    # Create Subcounty - Central division
    subcounty, _ = Subcounty.objects.get_or_create(county_municipality=county_municipality, name='Central division')

    # Create Parishes and Villages for each parish

    parishes_and_villages = [
        ('Ward ii', ['Ruhandagazi', 'Rushinya', 'Nyabicerere', 'Nyarwanya']),
        ('Ryamabengwa ward', ['Gabikye', 'Rweibare', 'Katungu', 'Ryamabengwa', 'Mutojo', 'Nyabicerere']),
        ('Ruharo ward', ['Kikuba central', 'Kayojo', 'Kicwamba', 'Kyabumbaire', 'Kamira', 'Mabaare', 'Rwandaro', 'Masya', 'Ruharo central', 'Nombe']),
        ('Kyeitembe ward', ['Bunyarigi', 'Rweibare', 'Kyakagina', 'Kashekye', 'Kyeitembe east', 'Kyeitembe west', 'Ruyayo', 'Twengoma', 'Tank hill']),
        ('Central ward', ['Bwatogo', 'Central']),
    ]
