...
def populate_database():
    ...
    # Create Subcounty - Kazo town council
    subcounty, _ = Subcounty.objects.get_or_create(county_municipality=county_municipality, name='Kazo town council')

    # Create Parishes and Villages for each parish

    parishes_and_villages = [
        ('Obwengara', ['Rwamanuuma', 'Rwanyamuzira', 'Obwengara']),
        ('Rwempiri', ['Kigorogoro', 'Rweminyafu', 'Rwempiri', 'Ruzinga']),
        ('Rwemirondo ward', ['Rwemirondo i', 'Rwemirondo ii', 'Karisizo']),
        ('Kazo ward', ['Bwesirabo', 'Kazo i', 'Kazo ii']),
        ('Gabarungi', ['Gabarungi', 'Rwamuyeye', 'Rwabwonyo']),
        ('Byeshembe ward', ['Byeshembe', 'Kyarugyendo']),
    ]
