...
def populate_database():
    ...
    # Create Subcounty - Sanga
    subcounty, _ = Subcounty.objects.get_or_create(county_municipality=county_municipality, name='Sanga')

    # Create Parishes and Villages for each parish

    parishes_and_villages = [
        ('Nombe i', ['Nyakigando']),
        ('Nombe ii', ['Kigarama (ranches 18 & 19)', 'Rwensheko (ranches 1 & 5)', 'Nyankumba (ranches 17, 20, 21, 22)', 'Rwakobo']),
        ('Rwabarata', ['Kiribwa', 'Rwonyo', 'Rwamuhuku', 'Rwabarata']),
    ]
