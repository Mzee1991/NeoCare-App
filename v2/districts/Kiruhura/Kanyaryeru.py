...
def populate_database():
    ...
    # Create Subcounty - Kanyaryeru
    subcounty, _ = Subcounty.objects.get_or_create(county_municipality=county_municipality, name='Kanyaryeru')

    # Create Parishes and Villages for each parish

    parishes_and_villages = [
        ('Rwamuranda', ['Rwamuranda', 'Obwapa']),
        ('Kibega', ['Kikoma', 'Kibega']),
        ('Kanyaryeru res sch', ['Kanyaryeru', 'Mpangamushanju']),
        ('Akayanja', ['Bumaranjara', 'Rwakobo ii', 'Rushororo']),
        ('Akaku', ['Rwendama', 'Rwozi']),
    ]
